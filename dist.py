...
def create_nodos_release(gh_release_repo, gh_release_notes, gh_release_title_postfix, gh_release_target_branch, dry_run_release):
    ...
    for path in artifacts_abspath:
        ...
        nosman_args = [f"nosman", "publish", "--path", path, "--name", nodos_package_name, "--version", f"{major}.{minor}.{patch}", "--version-suffix", f".b{build_number}", "--type", "nodos", "--vendor", "Nodos", "--publisher-name", "mediaz-bot", "--publisher-email",
                    "bot@nodos.dev"]
        if dry_run_release:
...
def build_nosman(nosman_src_dir, is_release) -> str:
    ...
    result = run(["upx", "--best", f"target/{'release' if is_release else 'debug'}/nosman"], stdout=stdout, stderr=stderr, universal_newlines=True)
    if result.returncode != 0:
