...
pub fn silently_agree_eulas(workspace_dir: &std::path::PathBuf) {
    ...
    for entry in std::fs::read_dir(&engines_dir).unwrap_or_else(|_| panic!("Unable to read Engine directory at {}", engines_dir.display())) {
        ...
        let eula_str = std::fs::read_to_string(&eula_unconfirmed).unwrap_or_else(|_| panic!("Failed to read EULA_UNCONFIRMED file: {}", eula_unconfirmed.display()));
        let mut eula_json: serde_json::Value = serde_json::from_str(&eula_str).unwrap_or_else(|_| panic!("Failed to parse EULA_UNCONFIRMED file: {}", eula_unconfirmed.display()));
        ...
        std::fs::write(&eula_unconfirmed, eula_json.to_string()).unwrap_or_else(|e| panic!("Failed to write EULA_UNCONFIRMED file {}: {}", eula_unconfirmed.display(), e));
        // Move the file to EULA_CONFIRMED.json
        let eula_confirmed = path.join("EULA_CONFIRMED.json");
        std::fs::rename(&eula_unconfirmed, &eula_confirmed).unwrap_or_else(|e| panic!("Failed to rename EULA_UNCONFIRMED file {}: {}", eula_unconfirmed.display(), e));
    }
