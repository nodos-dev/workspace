    ...
    pub fn run_install(&self, workspace: &mut Workspace, package_name: &str, version_opt: Option<&String>, output_dir: &Option<PathBuf>, prefix: Option<&String>, flags : InstallFlags) -> Result<InstallOp, CommandError> {
        ...
        if version_opt.is_some() {
            ...
            if !flags.contains(InstallFlags::InstallExactVersion) {
                let version_constraint = SemVer::parse_from_str(version.as_str()).unwrap_or_else(|| panic!("Failed to parse semantic version"));
                if let Some(installed_package) = workspace.get_latest_local_package_for_constraint(package_name, &version_constraint) {
        ...
        if !flags.contains(InstallFlags::InstallExactVersion) {
            ...
            let version_constraint = SemVer::parse_from_str(version.as_str()).unwrap_or_else(|| panic!("Failed to parse semantic version"));
            println!("Installing {} matching version constraint '{}'", package_name, version_constraint.to_string());
            return {
                let latest_compatible_opt = workspace.index_cache.get_latest_compatible_release(&package_name, &version_constraint);
                let compatible_package = if let Some((package_type, release)) = latest_compatible_opt {
                    ...
                    return Err(InvalidArgument { message: format!("No remote contained a version matching constraint '{}' for package {}", version_constraint.to_string(), package_name) });
                };
